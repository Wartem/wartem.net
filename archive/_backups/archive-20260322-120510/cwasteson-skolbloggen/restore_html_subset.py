...
def should_include(row: dict[str, str]) -> bool:
    ...
    if kind in {"homepage", "post_like", "category", "pagination"}:
        return True
    if kind == "other" and (MONTH_ARCHIVE_RE.match(path) or path in {"/om/", "/om-mig/", "/about/"}):
        return True
    return False
...
class Downloader:
    ...
    def fetch(self, url: str) -> bytes:
        request = Request(url, headers={"User-Agent": "cwasteson-html-subset/1.0", "Accept-Encoding": "identity"})
        attempt = 0
        while True:
            try:
