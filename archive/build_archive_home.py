...
def render_collection_buttons(collection: dict[str, Any]) -> str:
    buttons: list[str] = []
    for entry in collection.get("entries", []):
        local_path, _exists = local_target(entry)
        href = local_path or entry.get("canonical_url")
        if not href:
            continue
        buttons.append(render_link(href, entry["title"]))
    if not buttons:
        return ""
    return f'<div class="linklist">{"".join(buttons)}</div>'
...
<article class="card">
  ...
  <p><span class="{status_class(collection['status'])}">{escape(collection['status'])}</span></p>
  {render_collection_buttons(collection)}
